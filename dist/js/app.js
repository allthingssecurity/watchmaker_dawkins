...
window.onload = function () {
  ...
  // Simple global bag for cross-state data
  game.global = { levelIndex: 0 };
...
var Book = function(game) {
    ...
    // This Book implementation now controls an external DOM overlay for clarity
    // and to allow clean pausing of gameplay when visible.

    // Toast will be managed via DOM too (for crispness outside canvas)
...
Book.prototype.loadConcepts = function(jsonText) {
    try {
...
Book.prototype.toast = function(title) {
    var el = document.getElementById('concept-toast');
    if (!el) return;
    el.textContent = 'New Concept: ' + (title || '');
    el.style.opacity = '1';
    var t = setTimeout(function(){ el.style.opacity = '0'; }, 1500);
};

Book.prototype.show = function() {
    // Pause gameplay while book is open
    this.visible = true;
...
Book.prototype.hide = function() {
    ...
    // Clear keyboard state to avoid immediate re-toggle on key release
...
Book.prototype.addConcept = function(concept) {
    if (!concept) return;
    this.unlocked.push(concept);
};

Book.prototype.pageNext = function(){
    if (!this.unlocked.length) return;
    this.pageIndex = Math.min(this.unlocked.length-1, (this.pageIndex||0)+1);
    this.renderPage();
};

Book.prototype.pagePrev = function(){
    if (!this.unlocked.length) return;
    this.pageIndex = Math.max(0, (this.pageIndex||0)-1);
    this.renderPage();
};
...
var Bunny = function(game, x, y, frame) {
    ...
    // Reset angle to avoid being stuck off-screen after state restart
    this.angle = 0;
	// set initial health
...
// Static reset function to reset speed variables between levels
Bunny.resetSpeed = function() {
	NORMAL_SPEED = 100;
	POWER_SPEED = 300;
};
...
var Carrot = function(game, x, y) {
    ...
    // display wine bottle sprite instead of carrot
    this.frameName = "carrot.png"; // atlas frame now replaced with wine image
...
Menu.prototype = {
	create: function() {
		...
		else { str_sub = 'LEARNER\nDUDE'; str_prs = 'Press Space!';}
		...
		// Reuse titlescreen region but treat as book theme
		...
		// Use external objects spritesheet for a book-themed emblem
		var sel = null;
		try { sel = JSON.parse(this.game.cache.getText('selected_icons')).selected || null; } catch(e){}
		var frame = sel && sel.length ? sel[0].index : 0;
		this.drill = this.game.add.sprite(146,this.game.height - 98, 'objects', frame);
		this.drill.smoothed = false;
		this.drill.scale.x = 0.75;
		this.drill.scale.y = 0.75;
		
		// real bun - reset speed before creating to avoid carryover from previous level
		Bunny.resetSpeed();
		...
		this.title = this.game.add.text(8,8,'Watchmaker\nAcademy:', {fill : 'red'});
		...
		// If returning from a level, skip intro animation quickly
		if (this.game.global && this.game.global.levelIndex > 0) {
			// bypass bun animation and go straight to play on Space or click
			var quick = this.game.add.text(8, this.game.height - 24, 'Press Space to Continue', {fill:'white'});
			quick.font = 'Press Start 2P'; quick.fontSize = 12;
			var self = this;
			this.game.input.keyboard.addKey(Phaser.Keyboard.SPACEBAR).onDown.addOnce(function(){ self.playScene(); });
			this.game.input.onDown.addOnce(function(){ self.playScene(); });
		}
...
},{"../prefabs/Bunny.js":4}],13:[function(require,module,exports){
'use strict';
...
var Book = require('../prefabs/Book.js');
...
Play.prototype = {
	create: function() {
		// CRITICAL: Force complete world reset for level transitions
		...
		this.game.camera.setBounds(0, 0, this.game.width, 1024);

		// Debug: log level transition
		console.log('=== PLAY CREATE ===');
		console.log('Level:', this.game.global ? this.game.global.levelIndex : 0);
		console.log('Camera:', this.game.camera.x, this.game.camera.y);
		console.log('World bounds:', this.game.world.bounds);

		// Ensure game and physics are unpaused and DOM overlays are hidden on state start
		this.game.paused = false;
		if (this.game.physics && this.game.physics.arcade) {
			this.game.physics.arcade.isPaused = false;
		}
		try {
			var ov = document.getElementById('book-overlay');
			if (ov) ov.style.display = 'none';
			var toast = document.getElementById('concept-toast');
		...
		// initialize game + level variables
		this.cash = 0;
    ...
    var LEVELS = [
		...
		// create the player (reset speed variables for clean level start)
		Bunny.resetSpeed();
        this.bunny = new Bunny(this.game, this.game.width/2, -32);
		...
		// Debug camera after follow setup
		console.log('Camera after follow:', this.game.camera.x, this.game.camera.y);
		console.log('Bunny position:', this.bunny.x, this.bunny.y);
    ...
    // repurpose carrot pickup sound to 'hich' (hiccup)
    this.sfx.addMarker('hich', 0.704, 0.535, 1, false);
    this.sfx.addMarker('gem', 1.252, 0.340, 1, false);
        ...
        // Start music immediately (original behavior)
        this.sfx.play('coral');
		...
		// switch currency display to Rupee
		...
		// Level goals HUD
        this.goaltext = this.game.add.text(4, 4, '', {fill: 'white'});
        this.goaltext.font = "Press Start 2P";
        this.goaltext.fontSize = 12;
        this.goaltext.fixedToCamera = true;
        this.updateGoalsHUD();

        // Book overlay (concepts)
        this.book = new Book(this.game);
...
beginRun: function(){},
	update: function() {
		if (!this.dead) {
			// Ensure DOM close works even while paused: also check while unpaused to hide if requested
        if (this.toggleBookKey && this.toggleBookKey.justPressed(1)) {
            // Prevent game-level handler from fighting DOM handler while paused
			...
			if (this.chunkGroup.children.length > 0 && this.bunny.y > this.chunkGroup.children[this.chunkGroup.children.length - 1].y)
			{
				this.generateChunk();
			}
			...
			this.progress.depth = Math.max(this.progress.depth, Math.round((this.bunny.y>0?this.bunny.y:0)/10));
			...
			if (this.bunny.health < 0) {
				...
				this.pressspace = this.game.add.text(8, 340, 'F5 TO RETRY\n\nTHANKS FOR PLAYING!\nWATCHMAKER ACADEMY\nLEARNER DUDE EDITION\n\nCREDITS:\nORIGINAL: DREAM SHOW ADVENTURES\nADAPTATION: SHASHANK JAIN', {fill: 'white'});
	...
	levelComplete: function() {
        ...
        if (this.bunny && this.bunny.body) {
            ...
            this.bunny.body.velocity.y = 0;
            this.bunny.body.angularVelocity = 0;
        }
        if (this.dirtEmitter) this.dirtEmitter.on = false;
        if (this.tunnel) this.tunnel.on = false;
        if (this.tunnelborder) this.tunnelborder.on = false;
        if (this.drilldirt && this.drilldirt.animations) this.drilldirt.animations.stop();
        if (this.sfx && this.sfx.stop) { try { this.sfx.stop(); } catch(e){} }
        if (this.game.physics && this.game.physics.arcade) {
            this.game.physics.arcade.isPaused = true;
        }
		var ov = this.game.add.bitmapData(this.game.width, this.game.height);
		ov.context.fillStyle = 'rgba(0,0,0,0.75)';
		ov.context.fillRect(0, 0, this.game.width, this.game.height);
		var s = this.game.add.sprite(0,0,ov); s.fixedToCamera = true;
		s.inputEnabled = true;
        var t = this.game.add.text(24, 180, 'LEVEL COMPLETE!\nPress Space/Enter or Click', {fill:'white'});
        t.font = 'Press Start 2P'; t.fontSize = 24; t.fixedToCamera = true;
        var hint = this.game.add.text(24, 260, 'Press B to read the book', {fill:'#ffff88'});
        hint.font = 'Press Start 2P'; hint.fontSize = 14; hint.fixedToCamera = true;
		var self = this;
	    // ensure canvas focus and clear previous key state
	    if (this.game.canvas && this.game.canvas.focus) { this.game.canvas.focus(); }
	    this.game.input.keyboard.reset(true);
	    this.game.input.keyboard.addKeyCapture([Phaser.Keyboard.SPACEBAR, Phaser.Keyboard.B]);
    ...
    var enter = this.game.input.keyboard.addKey(Phaser.Keyboard.ENTER);
        ...
        k.onDown.addOnce(function(){ advance(); });
        enter.onDown.addOnce(function(){ advance(); });
        // also allow click/tap anywhere on overlay to advance
        s.events.onInputDown.addOnce(function(){ advance(); }, this);
        // Keep overlay-only click (sprite), avoid global onDown to reduce pointer signal issues
    // Offer to read the book up to this level
    b.onDown.addOnce(function(){
        self.book.show();
        // When closed, re-show the level complete overlay and wait for Space again
        var checkClose = self.game.time.events.loop(100, function(){
            if (!self.book.visible) {
                k.onDown.addOnce(function(){ advance(); });
                self.game.time.events.remove(checkClose);
            }
        });
    });
	},
	generateChunk: function() {
		...
		// Set camera bounds height based on current position (not accumulating)
		this.game.camera.bounds.height = this.nextChunkY + 512;
	...
	shutdown: function() {
		// Clean up for proper level transitions
		if (this.sfx) {
			this.sfx.stop();
			this.game.sound.remove(this.sfx);
		}
		if (this.dfx) {
			this.dfx.stop();
			this.game.sound.remove(this.dfx);
		}
		// Reset camera
...
},{"../prefabs/Block.js":2,"../prefabs/Book.js":3,"../prefabs/Bunny.js":4,"../prefabs/Carrot.js":5,"../prefabs/Gauge.js":6,"../prefabs/Gem.js":7,"../prefabs/Lava.js":8,"../prefabs/Rock.js":9}],14:[function(require,module,exports){
'use strict';
...
Preload.prototype = {
	preload: function() {
		...
		// Load external objects sprite sheet (icons) + selected indices
		this.load.spritesheet('objects', 'assets/objects.png', 128, 128);
		this.load.text('selected_icons', 'assets/icons/selected.json');
	},
	create: function() {
